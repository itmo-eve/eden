...
SWTPM_TAG ?= "lfedge/eden-swtpm"
# SWTPM_DIR is the directory with swtpm Dockerfile to build
SWTPM_DIR=$(CURDIR)/swtpm
# SWTPM_VERSION is the version of swtpm image to build
SWTPM_VERSION ?= $(shell git tag -l --contains HEAD)
ifeq ($(SWTPM_VERSION),)
	SWTPM_VERSION = $(shell git describe --always)
endif
...
push-multi-arch-swtpm:
	@echo "Build and $(DOCKER_TARGET) swtpm image $(SWTPM_TAG):$(SWTPM_VERSION)"
